...
#+begin_src shell :var dir="." :results replace raw
  cd $dir
  in=`tokei -C -o json`
  echo $in | jq -r '.["Total"] | .code, .comments, .blanks'
#+end_src
** tokei-dir-langs
...
#+begin_src shell :var dir="." :results replace raw
  cd $dir
  in=`tokei -C -o json`
  echo $in | jq -r '.["Total"].children | keys[]'
#+end_src

** sum-str-nums
#+name: sum-str-nums
#+begin_src emacs-lisp :var x=tokei-dir-lines
  (let ((tot 0))
    (cl-loop
     for i in (split-string x) do
